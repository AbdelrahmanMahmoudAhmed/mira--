			...
			<div class="picked">
				...
				<table>
					...
					<tbody>
						<tr v-for="(product, i) in myProducts" :key="i">
							<td class="td-x">
								<font-awesome-icon class="star" :icon="['fas', 'times']" @click="deleteProduct(product.id)" />
							</td>
							<td class="td-img">
								<router-link to="/products"><img :src="require(`@/assets/imgs/${product.img}`)" alt="" /></router-link>
							</td>
							<td class="td-details">
								<div>
									<router-link to="/products">
										...
										<span v-if="product.material">/ {{ product.material }} </span></router-link
									>
								</div>
							</td>
							<td class="td-price">
							...
							<td class="td-price the-total">
								$
								<span class="product-total">{{ (product.count * product.price).toFixed(2) }} </span>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<div class="buttons-holder">
					<div class="buttons-l">
						...
						<button @click="clearProducts">clear cart</button>
					</div>
					<div class="button-r">
	...
	setup() {
		...
		const calcTotal = () => {
			if (myProducts.value.length) {
				totalPrice.value = myProducts.value.reduce((curr, acc) => {
					return curr + acc.price * acc.count;
				}, 0);
			}
		};
		const deleteProduct = (e) => {
			store.commit("deleteProduct", e);
			myProducts.value = store.state.myProducts;
		};
		const clearProducts = () => {
		...
		onMounted(() => {
			window.scrollTo(0, 0);
			store.commit("newPageTitle", "YOUR SHOPPING CART");
