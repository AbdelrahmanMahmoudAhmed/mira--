<template>
	<div class="header">
		<div class="logo">
			<img src="https://cdn.shopify.com/s/files/1/2213/2375/files/logo_medium.png?v=1500972950" alt="" />
		</div>
		<div class="nav">
			<router-link to="/">
				HOME
			...
			/></router-link>
			<router-link to="/products">SHOP <font-awesome-icon class="nav-chev" :icon="['fas', 'chevron-down']" /></router-link>
		...
		<div class="headr-action">
			...
			<router-link to="/cart">
				<font-awesome-icon class="action-icons" :icon="['fas', 'cart-arrow-down']" />
			</router-link>
		</div>
	...
	setup() {
		...
		onMounted(() => {
			...
			window.addEventListener("click", function (e) {
				if (!links.value) {
					if (!e.target.closest(".nav-toggle") && e.target != document.querySelector(".media-container") && e.target != box) {
						let allSpans = document.querySelectorAll("span.for-c");

						document.getElementsByClassName("nav-toggle")[0].style.visibility = "hidden";
		...
		const toggleLinks = () => {
			...
			if (links.value) {
				document.getElementsByClassName("nav-toggle")[0].style.visibility = "visible";
